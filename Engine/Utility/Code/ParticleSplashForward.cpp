...
CParticleSplashForward::CParticleSplashForward(int numParticles, _float fSize)
{
	m_fSize = fSize;
	m_VbSize = 2048;
	m_VbOffset = 0;
...
CParticleSplashForward::~CParticleSplashForward()
{
	Free();
}

void CParticleSplashForward::Free()
{
...
bool CParticleSplashForward::Ready_Particle(IDirect3DDevice9* pDevice)
{
...
void CParticleSplashForward::Reset_Partice(Attribute* attribute)
{
	attribute->_bIsAlive = true;
	attribute->_vPosition = { m_matWorld._41, m_matWorld._42, m_matWorld._43 };

	m_vMin = _vec3(-0.5f, 0.f, -1.f);
	m_vMax = _vec3(0.5f, 0.f, 1.f);
...
void CParticleSplashForward::Update_Particle(_float fTimeDelat)
{
...
void CParticleSplashForward::Render_GameObject()
{
...
void CParticleSplashForward::Create_Texture(const _tchar* pTexturePath, _int iMaxFrame)
{
	m_iPicNum = iMaxFrame;
	m_pTexture = CTexture::Create(m_pGraphicDev, TEX_NORMAL, pTexturePath, iMaxFrame);
}

CParticleSplashForward* CParticleSplashForward::Create(IDirect3DDevice9* pDevice, 
	_matrix matWorld, 
	const _tchar* pTextruePath,
	_int iMaxFrame, _float fSize, _int iCount)
{
	CParticleSplashForward* pInstance = new CParticleSplashForward(iCount, fSize);
	pInstance->Set_World_Matrix(matWorld);

	if (FAILED(pInstance->Ready_Particle(pDevice)))
	{
		Safe_Release(pInstance);
		MSG_BOX("ParticleSplashForward Create Failed");
		return nullptr;
	}
