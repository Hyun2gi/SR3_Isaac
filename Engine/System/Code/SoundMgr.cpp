...
bool CSoundMgr::PlayEffect(TCHAR* pSoundKey, CHANNEL_ID eID, _float fVolume)
{
	...
	else if(SOUND_EFFECT_ETC_STOPSUDDEN == eID || SOUND_EFFECT_PLAYER_STOPSUDDEN == eID || SOUND_EFFECT_ITEM_STOPSUDDEN == eID
		|| SOUND_EFFECT_MON_STOPSUDDEN == eID || SOUND_EFFECT_OBJ_STOPSUDDEN == eID || SOUND_EFFECT_MON_STOPSUDDEN_SUB == eID
		|| SOUND_BGM_EARTHQUAKE == eID || SOUND_EFFECT_BOSS_STOPSUDDEN == eID || SOUND_EFFECT_BOSS_STOPSUDDEN_SUB == eID)
	{
		StopSound(eID);
		FMOD_System_PlaySound(m_pSystem, FMOD_CHANNEL_FREE, iter->second, FALSE, &m_pChannelArr[eID]);
	}
...
void CSoundMgr::SetChannelVolume(CHANNEL_ID eID, _float fVolume)
{
	FMOD_Channel_SetVolume(m_pChannelArr[eID], fVolume);
